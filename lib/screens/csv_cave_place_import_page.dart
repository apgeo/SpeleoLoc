...
class CSVCavePlacesImportPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CSVCavePlacesImportPage> createState() =>
      _CSVCavePlacesImportPageState();
}

class _CSVCavePlacesImportPageState
    extends ConsumerState<CSVCavePlacesImportPage> {
  ...
  Future<void> _navigateToCSVImport() async {
    ...
    final result = await Navigator.push<CSVImportResult>(
      ...
      MaterialPageRoute(
        builder: (_) =>
            CSVImportPage(title: title, columnDefinitions: _columnDefinitions),
      ),
    );
  ...
  Future<void> _processImport(CSVImportResult csvResult) async {
    ...
    try {
      final config = CSVCavePlacesImportConfig(
        ...
        caveNameColumn: _isMultipleCaveMode
            ? csvResult.columnMappings['cave_name']
            : null,
        cavePlaceNameColumn: csvResult.columnMappings['place_name'],
        qrCodeColumn: csvResult.columnMappings['qr_code'],
        caveAreaColumn: _isMultipleCaveMode
            ? csvResult.columnMappings['cave_area']
            : null,
  ...
  Future<bool?> _showQrConflictDialog(
    List<CavePlaceExistingMatch> conflicts,
  ) async {
    return showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              ...conflicts
                  .take(widget.maxPreviewDuplicates)
                  .map(
                    (c) => Padding(
                      padding: const EdgeInsets.symmetric(vertical: 2),
                      child: Text(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text(
              '${LocServ.inst.t('csv_cave_areas_created')}: ${result.caveAreasCreated}',
            ),
            Text(
              '${LocServ.inst.t('csv_cave_places_created')}: ${result.cavePlacesCreated}',
            ),
            Text(
              '${LocServ.inst.t('csv_qr_codes_updated')}: ${result.qrCodesUpdated}',
            ),
          ],
        ),
        actions: [
